 ...
 * Each object contains the language code, its native name, and the path to its flag icon.
 * `as const` is used to treat this array as a readonly tuple, providing strict type safety.
 */
 ...
 * This type is derived automatically from the `a_languages` constant.
 */
export type Language = typeof a_languages[number];

/**
 * Represents the type for a language code (e.g., 'es', 'en').
 * This type is derived from the `Language` type for maximum type safety.
 */
