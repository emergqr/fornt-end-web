...
/**
 * @file A reusable form component for creating and editing contacts.
 */
...
const getContactSchema = (t: (key: string) => string) => z.object({
  ...
  relationship_type: z.string().min(2, { message: t('validation.relationshipRequired') }),
...
export default function ContactForm({ onSubmit, onCancel, initialData }: ContactFormProps) {
  ...
  const {
    ...
    defaultValues: initialData || { name: '', email: '', phone: '', relationship_type: '', is_emergency_contact: false },
  });

  React.useEffect(() => {
    reset(initialData || { name: '', email: '', phone: '', relationship_type: '', is_emergency_contact: false });
  }, [initialData, reset]);
